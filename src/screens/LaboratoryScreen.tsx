...
import { ScrollView, StyleSheet, Text, View, ActivityIndicator, TouchableOpacity, useWindowDimensions, Platform, Alert, TextInput } from 'react-native';
import { MaterialCommunityIcons } from '@expo/vector-icons';
import { C, KpiCard, ActionButton, AnimatedPage, FormModal, FormInput, FormSelect, SectionTitle, ExportOverlay, PaginationControls } from '../components/Ui';
import { useInstruments, useFcqDossiers, useUserProfile, useMutation, useNotification, useLots, usePermissions, useQcSpecifications, useCalibrationLog } from '../lib/hooks';
...
export function LaboratoryScreen() {
  ...
  const { data: dossiers = EMPTY_ARRAY, count: dossiersCount, isPending: dossLoading } = useFcqDossiers(page, limit);
  ...
  const [freeParams, setFreeParams] = React.useState<Array<{ key: string; name: string; unit: string; value: string; isDefault: boolean; norm?: string }>>([]);
  ...
  const { data: specs = EMPTY_ARRAY, isPending: specsLoading } = useQcSpecifications(specRef || undefined);
  ...
  React.useEffect(() => {
    if (dossier && dossier.results) {
      ...
      const free: Array<{ key: string; name: string; unit: string; value: string; isDefault: boolean; norm?: string }> = [];
      Object.entries(dossier.results).forEach(([k, v]) => {
        if (k.startsWith('__free__')) {
          ...
          free.push({ key: k, name, unit: defParam?.unit ?? '', value: String(v), isDefault: !!defParam, norm: String(normValue) });
        }
      });
  ...
  React.useEffect(() => {
    ...
    const defaults = DEFAULT_ANALYSIS_PARAMS
      .filter(p => DEFAULT_VISIBLE_PARAM_NAMES.includes(p.name))
      .map(p => ({ key: buildFreeKey(p.name), name: p.name, unit: p.unit, value: '', isDefault: true }));
  ...
  React.useEffect(() => {
    if (dossier) {
      setControlledQty(dossier.quantite_controlee ? dossier.quantite_controlee.toString() : (dossier.lot?.qty_received ? dossier.lot.qty_received.toString() : ''));
  ...
  const [decisionFilter, setDecisionFilter] = React.useState<'ALL' | 'LIBERE' | 'BLOQUE'>('ALL');
  ...
  const handleAdd = () => {
    ...
    if (operationalInstruments.length === 0) {
      Alert.alert("Accès Bloqué", "Aucun instrument n'est à jour d'étalonnage. Saisie FCQ impossible (Règle §4.5).");
      return;
    }
  ...
  const handleCompleteAnalysis = () => {
    ...
    if (inst && inst.status !== 'ETALONNE') {
      Alert.alert("Conformité", "L'instrument utilisé est hors période d'étalonnage. Résultats non validables.");
      return;
    }
  ...
  const renderResultInput = (spec: any) => {
    ...
    const isOut = rawVal !== undefined && rawVal !== '' && (val < spec.min_value || val > spec.max_value);
    ...
    return (
      <View key={spec.id} style={s.resRow}>
        <View style={{ flex: 1 }}>
          <Text style={s.resLabel}>{spec.parameter_name} ({spec.unit})</Text>
          <Text style={s.resSpec}>{t('target')}: {spec.min_value.toFixed(3)} - {spec.max_value.toFixed(3)}</Text>
        </View>
        {isReadOnly ? (
          <View style={[
            s.resInput,
  ...
  const generateFcqPdf = async () => {
    ...
    try {
      ...
      if (specs.length > 0) {
        resultsHtml = specs.map(spec => {
          const rawPdf = results[spec.id];
          const val = parseFloat(rawPdf || '0');
          const isOut = rawPdf !== undefined && rawPdf !== '' && !isNaN(val) && (val < spec.min_value || val > spec.max_value);
        ...
        resultsHtml = freeParams.map(param => {
          return `
            <tr>
      ...
      const htmlContent = getPdfTemplate(
        dossier.decision === 'LIBERE' ? 'CERTIFICAT DE CONFORMITÉ QUALITÉ' : 'RAPPORT D\'ANALYSE ET DE BLOCAGE',
        `
        ...
        `,
        { 
          watermark: dossier.decision === 'LIBERE' ? 'CONFORME' : dossier.decision === 'BLOQUE' ? 'REJETÉ' : 'BROUILLON',
  ...
  const handleDecision = (decision: 'LIBERE' | 'BLOQUE') => {
    ...
    if (!decisionMotive.trim()) {
      Alert.alert("Motif obligatoire", "Veuillez renseigner le motif de la décision (libération ou blocage).");
      return;
    }

    const parsedQty = parseFloat(controlledQty);
    if (isNaN(parsedQty) || parsedQty <= 0) {
      Alert.alert("Quantité invalide", "Veuillez renseigner une quantité contrôlée valide supérieure à 0.");
      return;
    }
    ...
    if (specs.length === 0 && decision === 'LIBERE') {
      const hasFreeData = Object.keys(results).some(k => k.startsWith('__free__') && results[k] !== '');
      if (!hasFreeData) {
    ...
    if (decision === 'LIBERE') {
      ...
      if (outOfSpecParams.length > 0) {
        const paramNames = outOfSpecParams.map(s => `• ${s.parameter_name} (cible: ${s.min_value}–${s.max_value} ${s.unit || ''})`).join('\n');
        Alert.alert(
  ...
  const _executerDecision = (decision: 'LIBERE' | 'BLOQUE', outOfSpecParams: any[]) => {
    ...
    mutation.mutate({
      ...
      onSuccess: async () => {
        ...
        if (decision === 'BLOQUE') {
          try {
            ...
            const { error: fncErr } = await supabase!.from('fnc').insert({
              ...
              lot_id: dossier.lot_id,
              fcq_id: dossier.id,
              lot_code: dossier.lot?.code ?? null,
              article_name: dossier.lot?.article?.name ?? null,
              severity: 'MAJEURE',
        ...
        const decisionLabel   = decision === 'LIBERE' ? 'LOT LIBERE — Conforme' : 'LOT BLOQUE — Non-conforme';
        ...
        const rolesDecision = decision === 'LIBERE'
  ...
  return (
    <AnimatedPage>
      {isGeneratingPdf && <ExportOverlay visible={true} progress={0.5} />}{/* Simple overlay for PDF generation */}
      <ScrollView style={s.container} contentContainerStyle={s.content}>
        ...
        <View style={[s.headerRow, isMobile && { flexDirection: 'column', alignItems: 'flex-start', gap: 16 }]}>
          ...
          <View style={s.actions}>
            <ActionButton label={t('calibration_planning')} onPress={() => setCalibModalVisible(true)} />
            {canCreateFcq && (
        ...
        <View style={[s.mainGrid, isMobile && { flexDirection: 'column' }]}>
            ...
            <View style={s.card}>
              {instruments.map((inst, i) => (
                <View key={inst.id} style={[s.instRow, i < instruments.length - 1 && s.borderB]}>
                  ...
                  <View style={[s.statusDot, { backgroundColor: inst.status === 'ETALONNE' ? C.ok : C.err }]} />
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={[s.rightCol, isMobile && { width: '100%' }]}>
            <View style={s.tableCard}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8, padding: 12 }}>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  <TouchableOpacity onPress={() => setDecisionFilter('ALL')} style={{ paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, backgroundColor: decisionFilter === 'ALL' ? '#111' : '#F8F9FA' }}>
                    <Text style={{ color: decisionFilter === 'ALL' ? '#FFF' : '#6C757D', fontWeight: '700' }}>Tous</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => setDecisionFilter('LIBERE')} style={{ paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, backgroundColor: decisionFilter === 'LIBERE' ? C.ok : '#F8F9FA' }}>
                    <Text style={{ color: decisionFilter === 'LIBERE' ? '#FFF' : '#6C757D', fontWeight: '700' }}>Libéré</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => setDecisionFilter('BLOQUE')} style={{ paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, backgroundColor: decisionFilter === 'BLOQUE' ? C.err : '#F8F9FA' }}>
                    <Text style={{ color: decisionFilter === 'BLOQUE' ? '#FFF' : '#6C757D', fontWeight: '700' }}>Bloqué</Text>
                  </TouchableOpacity>
                </View>
                <View />
              </View>
              ...
              {filteredDossiers.map((d, idx) => (
                <TouchableOpacity
                  ...
                  style={[s.tr, selId === d.id && s.trActive, idx === dossiers.length - 1 && { borderBottomWidth: 0 }]}
                >
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      <Text style={s.tdCode}>{d.code}</Text>
                      <View style={s.miniBadge}><Text style={s.miniBadgeText}>{d.fcq_type}</Text></View>
                    </View>
                  ...
                  <View style={{ width: 140, alignItems: 'flex-end', gap: 8 }}>
                    <View style={[s.statusBadge, { backgroundColor: STATUS_MAP[d.status]?.color + '15' }]}>
                      <Text style={[s.statusText, { color: STATUS_MAP[d.status]?.color }]}>{STATUS_MAP[d.status]?.label}</Text>
                    </View>
                    {d.decision && (
                      <Text style={[s.decisionText, { color: d.decision === 'LIBERE' ? C.ok : C.err }]}>
                        {d.decision}
                      </Text>
                    )}
                  </View>
            ...
            {dossier && (() => {
              ...
              const isWaiting = dossier.status === 'EN_ATTENTE' || (!hasAnyResult && dossier.status !== 'VALIDE' && dossier.status !== 'COMPLET');
              ...
              const isArticleMP = dossier.fcq_type === 'FCQ-MP' || dossier.lot?.article?.article_type === 'MP';
                  ...
                  <View style={s.detailContent}>
                    ...
                    <View style={s.metaGrid}>
                      ...
                      <View style={s.metaItem}>
                        ...
                        <Text style={[s.metaValue, { fontFamily: Platform.OS === 'web' ? 'Menlo' : 'monospace', fontWeight: 'bold' }]}>
                      ...
                      <View style={s.metaItem}>
                        ...
                        <Text style={[s.metaValue, { fontFamily: Platform.OS === 'web' ? 'Menlo' : 'monospace', color: C.info, fontWeight: 'bold' }]}>
                          {dossier.lot?.code || 'N/A'}
                        </Text>
                      </View>
                      <View style={s.metaItem}>
                        <Text style={s.metaLabel}>Unité de mesure :</Text>
                        <Text style={s.metaValue}>{dossier.lot?.unit || dossier.lot?.article?.unit || 'kg'}</Text>
                      </View>
                      <View style={s.metaItem}>
                        <Text style={s.metaLabel}>Date Réception / Prod :</Text>
                        <Text style={s.metaValue}>
                          {new Date(dossier.lot?.reception_date || dossier.lot?.created_at || dossier.created_at).toLocaleDateString('fr-FR')}
                        </Text>
                      </View>
                    </View>
                    ...
                    <View style={s.qcResultHeader}>
                      ...
                      <Text style={[s.qcResultValue, { color: qcResultColor }]}>{qcResultLabel}</Text>
                    </View>
                    ...
                    <View style={s.resContainer}>
                      {specsLoading ? (
                        ...
                        <View>
                          {/* En-tête info */}
                          <View style={{ backgroundColor: '#EEF2FF', borderRadius: 6, padding: 10, marginBottom: 12, borderLeftWidth: 3, borderLeftColor: C.info }}>
                            <Text style={{ fontSize: 12, color: '#1A56DB', fontWeight: '700', marginBottom: 2 }}>
                              Paramètres d'analyse — saisie manuelle
                            </Text>
                            <Text style={{ fontSize: 11, color: '#374151' }}>
                              Les paramètres ci-dessous sont pré-remplis avec les valeurs standards du laboratoire. Modifiez les noms ou valeurs selon l'article, et ajoutez d'autres mesures si nécessaire.
                            </Text>
                          </View>

                          {/* En-tête de colonne */}
                          <View style={{ flexDirection: 'row', paddingHorizontal: 4, marginBottom: 4, gap: 8 }}>
                            <Text style={{ flex: 1, fontSize: 10, fontWeight: '800', color: '#9CA3AF', letterSpacing: 0.5 }}>PARAMÈTRE</Text>
                            <Text style={{ width: 90, fontSize: 10, fontWeight: '800', color: '#9CA3AF', letterSpacing: 0.5, textAlign: 'center' }}>VALEUR</Text>
                            <Text style={{ width: 44, fontSize: 10, fontWeight: '800', color: '#9CA3AF', letterSpacing: 0.5, textAlign: 'center' }}>UNITÉ</Text>
                            <View style={{ width: 28 }} />
                          </View>
                          ...
                          {freeParams.map((param, idx) => {
                            ...
                            return (
                              ...
                              >
                                  ...
                                  {isReadOnly || param.isDefault ? (
                                    <View style={{ flexDirection: 'column', gap: 2 }}>
                                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                                        {param.isDefault && (
                                          <View style={{ backgroundColor: '#EEF2FF', paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4 }}>
                                            <Text style={{ fontSize: 9, fontWeight: '700', color: C.info }}>STD</Text>
                                          </View>
                                        )}
                                        <Text style={{ fontSize: 13, fontWeight: '700', color: '#1A1A1A' }}>{param.name}</Text>
                                      </View>
                                      {isReadOnly ? (
                                        param.norm ? <Text style={{ fontSize: 11, color: '#6B7280', fontStyle: 'italic' }}>Norme: {param.norm}</Text> : null
                                      ) : (
                                        <TextInput
                                          style={{ fontSize: 11, color: '#6B7280', fontStyle: 'italic', paddingVertical: 0, marginTop: 2, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }}
                                          value={param.norm || ''}
                                          onChangeText={(txt) => {
                                    ...
                                    <View style={{ flexDirection: 'column', gap: 2 }}>
                                      <TextInput
                                        style={{ fontSize: 13, fontWeight: '700', color: '#1A1A1A', borderBottomWidth: 1, borderBottomColor: '#D1D9E0', paddingVertical: 2 }}
                                        value={param.name ?? ''}
                                        onChangeText={(txt) => {
                                ...
                                {/* Valeur */}
                                {isReadOnly ? (
                                  <View style={[s.resInput, { justifyContent: 'center', backgroundColor: hasValue ? '#F0FFF4' : '#F8F9FA', borderColor: hasValue ? C.ok : '#D1D9E0', width: 90 }]}>
                                    <Text style={{ textAlign: 'right', fontWeight: '800', fontSize: 14, color: hasValue ? '#15803D' : '#ADB5BD' }}>
                                      {param.value || '—'}
                                    </Text>
                                  </View>
                                ) : (
                                ...
                                <View style={{ width: 44, alignItems: 'center' }}>
                                  <Text style={{ fontSize: 12, color: '#6C757D', fontWeight: '600' }}>{(param as any).unit || ''}</Text>
                                </View>
                            ...
                            <View style={{ marginTop: 12, gap: 8 }}>
                              ...
                              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
                                {DEFAULT_ANALYSIS_PARAMS
                                  .filter(p => !freeParams.some(fp => fp.name === p.name))
                                  .map(p => (
                                    <TouchableOpacity
                                      key={p.name}
                                      onPress={() => {
                                        const key = buildFreeKey(p.name);
                                        setFreeParams(prev => [...prev, { key, name: p.name, unit: p.unit, value: '', isDefault: true }]);
                                      }}
                                      style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16, backgroundColor: '#F0F9FF', borderWidth: 1, borderColor: '#BAE6FD' }}
                                    >
                                      <MaterialCommunityIcons name="plus" size={12} color="#0284C7" />
                                      <Text style={{ fontSize: 11, color: '#0284C7', fontWeight: '700' }}>{p.name}{p.unit ? ` (${p.unit})` : ''}</Text>
                                    </TouchableOpacity>
                                  ))
                                }
                              </View>
                              ...
                              <TouchableOpacity
                                onPress={() => {
                                  ...
                                  setFreeParams(prev => [...prev, { key, name: '', unit: '', value: '', isDefault: false }]);
                                }}
                                style={{ flexDirection: 'row', alignItems: 'center', gap: 6, padding: 10, borderRadius: 6, borderWidth: 1, borderColor: '#9CA3AF', borderStyle: 'dashed', justifyContent: 'center' }}
                              >
                                <MaterialCommunityIcons name="pencil-plus-outline" size={16} color="#6C757D" />
                                <Text style={{ fontSize: 13, color: '#6C757D', fontWeight: '600' }}>Ajouter un paramètre personnalisé</Text>
                              </TouchableOpacity>
                            </View>
                          )}
                        </View>
                    ...
                    {dossier.status !== 'VALIDE' ? (
                      <View>
                        ...
                        {dossier.status !== 'COMPLET' && canCreateFcq && (
                          <View style={{ marginBottom: 24, padding: 16, backgroundColor: '#FFFDF5', borderRadius: 8, borderWidth: 1, borderColor: '#FFEAA7' }}>
                            <SectionTitle>{t('technician_action')}</SectionTitle>
                            <Text style={{ fontSize: 12, color: '#666', marginBottom: 12 }}>
                              Renseignez toutes les valeurs physiques. Enregistrez en brouillon à tout moment, puis finalisez quand la saisie est complète.
                            </Text>
                        ...
                        {canValidate ? (
                          <View style={s.rqFormCard}>
                            <SectionTitle>3. FORMULAIRE DE DÉCISION CQ-LIB (RESPONSABLE QUALITÉ)</SectionTitle>
                            ...
                            <View style={s.formField}>
                              <Text style={s.fieldLabel}>Motif de Libération ou de Blocage *</Text>
                              <TextInput
                                style={[s.formTextInput, { borderColor: decisionMotive.trim() ? '#D1D9E0' : C.gold }]}
                            ...
                            <View style={s.formField}>
                              <Text style={s.fieldLabel}>Commentaire / Observations additionnelles</Text>
                              <TextInput
                            ...
                            <View style={s.formField}>
                              ...
                              <Text style={s.autoTimestamp}>Automatique lors de la décision finale</Text>
                            </View>
                            ...
                            {specs.length > 0 ? (
                              <View style={{
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                  <MaterialCommunityIcons name={specs.some(sp => { const v = parseFloat(results[sp.id] || ''); return !isNaN(v) && (v < sp.min_value || v > sp.max_value); }) ? "alert-circle" : "check-circle"} size={16} color={specs.some(sp => { const v = parseFloat(results[sp.id] || ''); return !isNaN(v) && (v < sp.min_value || v > sp.max_value); }) ? C.err : C.ok} />
                                  <Text style={{ fontSize: 13, color: '#333', fontWeight: '500' }}>
                                    {specs.some(sp => { const v = parseFloat(results[sp.id] || ''); return !isNaN(v) && (v < sp.min_value || v > sp.max_value); })
                                      ? `${specs.filter(sp => { const v = parseFloat(results[sp.id] || ''); return !isNaN(v) && (v < sp.min_value || v > sp.max_value); }).length} paramètre(s) HORS SPEC - "${dossier.lot?.article?.name}"`
                                      : `Tous les paramètres conformes aux spécifications de "${dossier.lot?.article?.name}"`
                                    }
                                  </Text>
                                </View>
                                {hasConformityIssues && specs.filter(sp => { const v = parseFloat(results[sp.id] || ''); return !isNaN(v) && (v < sp.min_value || v > sp.max_value); }).map(sp => (
                                  <Text key={sp.id} style={{ fontSize: 11, color: C.err, marginBottom: 2 }}>
                                    {`• ${sp.parameter_name}: ${results[sp.id]} (cible ${sp.min_value}–${sp.max_value} ${sp.unit || ''})`}
                                  </Text>
                                ))}
                              </View>
                            ) : (
                              <View style={{ backgroundColor: '#EEF2FF', padding: 12, borderRadius: 6, marginBottom: 12, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                <MaterialCommunityIcons name="information-outline" size={20} color={C.info} />
                                <Text style={{ fontSize: 13, color: '#1E3A8A', flex: 1, lineHeight: 20 }}>
                            ...
                            {(!controlledQty.trim() || !controllerName.trim() || !decisionMotive.trim()) && (
                              <View style={{ backgroundColor: '#FFF8E1', borderRadius: 8, padding: 10, marginBottom: 16 }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                  <MaterialCommunityIcons name="alert-outline" size={16} color="#856404" />
                                  <Text style={{ fontSize: 12, color: '#856404', fontWeight: '600' }}>Champs requis avant validation :</Text>
                                </View>
                                <Text style={{ fontSize: 12, color: '#856404' }}>- Quantité contrôlée{"\n"}- Décision (Libéré / Bloqué){"\n"}- Motif détaillé</Text>
                              </View>
                            )}

                            <View style={s.decActions}>
                              <ActionButton label={t('export')} onPress={generateFcqPdf} icon="file-pdf-box" disabled={isGeneratingPdf} />
                              <ActionButton
                                label={t('release_lot')}
                          ...
                          <View style={s.restrictedBox}>
                            {dossier.status === 'COMPLET' ? (
                              ...
                              <Text style={s.restrictedText}>Validation réservée aux Responsables Qualité (RQ).</Text>
                            )}
                          </View>
                        )}
                      </View>
                      ...
                      <View style={s.validationDetailsCard}>
                        ...
                        <View style={s.detailRow}>
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, backgroundColor: dossier.decision === 'LIBERE' ? C.bgOk : C.bgErr }}>
                            <MaterialCommunityIcons name={dossier.decision === 'LIBERE' ? 'check-circle' : 'close-circle'} size={18} color={dossier.decision === 'LIBERE' ? C.ok : C.err} />
                            <Text style={{ fontSize: 14, fontWeight: '800', color: dossier.decision === 'LIBERE' ? C.ok : C.err }}>
                        ...
                        <View style={s.detailRow}>
                          ...
                          <Text style={s.detailValue}>{dossier.quantite_controlee || dossier.lot?.qty_received || 'N/A'} {dossier.lot?.unit || 'kg'}</Text>
                        </View>

                        <View style={s.detailRow}>
                          <Text style={s.detailLabel}>Contrôleur Qualité :</Text>
                          <Text style={s.detailValue}>{dossier.controleur_nom || 'Responsable Qualité'}</Text>
                        </View>

                        <View style={s.detailRow}>
                          <Text style={s.detailLabel}>Motif de la Décision :</Text>
                          <Text style={[s.detailValue, { fontStyle: 'italic', color: '#333' }]}>{dossier.motif_decision || 'Aucun motif renseigné'}</Text>
                        </View>
                        ...
                        <View style={s.detailRow}>
                          ...
                          <Text style={s.detailValue}>
                            {dossier.validated_at || dossier.validator_signed_at ? new Date(dossier.validated_at || dossier.validator_signed_at).toLocaleString('fr-FR') : 'N/A'}
                          </Text>
                        </View>

                        <View style={{ height: 16 }} />
                        <ActionButton label="Télécharger Certificat PDF" onPress={generateFcqPdf} icon="file-pdf-box" variant="secondary" />
                      </View>
                    )}
                  </View>
                </View>
      ...
      >
        ...
        <FormSelect
          ...
          options={[
            { label: 'QC MP', value: 'FCQ-MP' },
            { label: 'QC PF', value: 'FCQ-PF' },
            { label: 'QC SF', value: 'FCQ-SF' },
          ]}
      ...
      <FormModal
        ...
        onSave={() => {
          if (!calibFormData.instrument_id || !calibFormData.calibration_date || !calibFormData.next_due_date) {
      ...
      >
        ...
        <FormInput
          ...
          placeholder="YYYY-MM-DD"
        />
        <FormInput label="Étalon utilisé" value={calibFormData.standard_used ?? ''} onChangeText={v => setCalibFormData({ ...calibFormData, standard_used: v })} />
        <FormSelect
          label="Type d'étalon"
        ...
        />
        <FormInput label="Remarques" value={calibFormData.notes ?? ''} onChangeText={v => setCalibFormData({ ...calibFormData, notes: v })} multiline />
      </FormModal>
      ...
      >
        {calibLog.slice(0, 10).map((log: any) => (
          <View key={log.id} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#F1F3F5' }}>
            <Text style={{ fontSize: 13, fontWeight: '600' }}>{log.instrument?.code}</Text>
            <Text style={{ fontSize: 12, color: '#6C757D' }}>{new Date(log.calibration_date).toLocaleDateString()} → {new Date(log.next_due_date).toLocaleDateString()}</Text>
          </View>
        ))}
      </FormModal>
    </AnimatedPage>
...
const s = StyleSheet.create({
  ...
  card: { backgroundColor: '#FFF', borderRadius: 8, borderWidth: 1, borderColor: '#E9ECEF', padding: 16 },
  instRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12 },
  borderB: { borderBottomWidth: 1, borderBottomColor: '#F8F9FA' },
  instId: { fontSize: 11, fontWeight: '700', color: '#ADB5BD', fontFamily: Platform.OS === 'web' ? 'Menlo' : 'monospace' },
  ...
  tr: { flexDirection: 'row', padding: 20, borderBottomWidth: 1, borderBottomColor: '#F8F9FA', alignItems: 'center' },
  ...
  miniBadge: { backgroundColor: '#F8F9FA', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, borderWidth: 1, borderColor: '#E9ECEF' },
  miniBadgeText: { fontSize: 9, fontWeight: '800', color: '#1A1A1A' },
  detailCard: { marginTop: 24, backgroundColor: '#FFF', borderRadius: 8, borderWidth: 1, borderColor: '#E9ECEF', overflow: 'hidden' },
  ...
  closedBox: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, backgroundColor: '#F8F9FA', borderRadius: 6 },
  closedText: { fontSize: 13, fontWeight: '600', color: '#1A1A1A' },
  resContainer: { backgroundColor: '#F8F9FA', borderRadius: 8, padding: 12, gap: 8 },
  resRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', padding: 12, borderRadius: 6, borderWidth: 1, borderColor: '#E9ECEF' },
  resLabel: { fontSize: 13, fontWeight: '700', color: '#1A1A1A' },
  resSpec: { fontSize: 11, color: '#6C757D', marginTop: 2 },
  resInput: { width: 100, height: 36, borderWidth: 1, borderColor: '#D1D9E0', borderRadius: 4, paddingHorizontal: 8, fontSize: 14, textAlign: 'right', fontWeight: '600' },
  resInputError: { borderColor: C.err, color: C.err, backgroundColor: '#FFF5F5' },
  resStatus: { width: 40, alignItems: 'flex-end' },
  metaGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, backgroundColor: '#F8F9FA', padding: 16, borderRadius: 8, borderWidth: 1, borderColor: '#E9ECEF' },
  ...
  qcResultHeader: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8F9FA', padding: 12, borderRadius: 6, marginBottom: 12, borderWidth: 1, borderColor: '#E9ECEF' },
  qcResultLabel: { fontSize: 13, fontWeight: '700', color: '#1A1A1A', marginRight: 8 },
  qcResultValue: { fontSize: 14, fontWeight: '800' },
  rqFormCard: { backgroundColor: '#FFF8F5', borderRadius: 8, borderWidth: 1, borderColor: '#FFE8D6', padding: 16 },
  formField: { marginBottom: 12 },
  fieldLabel: { fontSize: 12, fontWeight: '700', color: '#495057', marginBottom: 4 },
  formTextInput: { backgroundColor: '#FFF', borderWidth: 1, borderColor: '#CED4DA', borderRadius: 6, paddingHorizontal: 12, paddingVertical: 8, fontSize: 14 },
  autoTimestamp: { fontSize: 13, color: '#6C757D', fontStyle: 'italic' },
  validationDetailsCard: { backgroundColor: '#F4FBF7', borderRadius: 8, borderWidth: 1, borderColor: '#D3F9E8', padding: 16 },
});
