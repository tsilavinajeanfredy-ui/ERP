...
function FcqGlobalResult({ lotId }: { lotId: string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (!results || results.length === 0 || results.some(r => r.is_conform === null)) {
        setResult({ label: 'EN ATTENTE', color: '#F5A623', icon: 'clock' });
        return;
      }
...
export function ReceptionPFScreen() {
  ...
  const [activeTab, setActiveTab] = React.useState<string>('QUARANTAINE');
  ...
  return (
    <AnimatedPage>
      <ScrollView style={s.container} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        {/* KPI Cards */}
        <View style={s.kpiRow}>
          <KpiCard label="Total Lots PF" value={String(kpiAll)} icon="package-variant" color={C.primary} />
          <KpiCard label="En Attente" value={String(kpiEA)} icon="clock-outline" color="#F5A623" />
          <KpiCard label="En Quarantaine" value={String(kpiQ)} icon="lock-outline" color={C.gold} />
          <KpiCard label="Libérés" value={String(kpiLib)} icon="check-circle-outline" color={C.ok} />
          <KpiCard label="Bloqués" value={String(kpiBlq)} icon="close-circle-outline" color={C.danger} />
        </View>
                  ...
                  {isSelected && (
                    <View style={s.cardDetail}>
                        ...
                        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                          ...
                          <ActionButton
                            ...
                            onPress={() => {
                              ...
                              () => deleteMutation.mutate({ id: lot.id, type: 'DELETE' })
                            );
                            }}
                          />
                        </View>
                      ...
                      <View style={s.detailRow}>
                        <Text style={s.detailLabel}>{t('internal_lot_number')}</Text>
                        <Text style={s.detailValue}>{lot.code}</Text>
                      </View>
                      <View style={s.detailRow}>
                        <Text style={s.detailLabel}>{t('article_label')}</Text>
                        <Text style={s.detailValue}>{lot.article?.name || '—'}</Text>
                      </View>
                      <View style={s.detailRow}>
                        <Text style={s.detailLabel}>{t('qty_received')}</Text>
                        <Text style={s.detailValue}>{lot.qty_received} {lot.unit}</Text>
                      </View>
                      <View style={s.detailRow}>
                        <Text style={s.detailLabel}>{t('reception_date')}</Text>
                        <Text style={s.detailValue}>{lot.reception_date || '—'}</Text>
                      </View>
                      <View style={s.detailRow}>
                        <Text style={s.detailLabel}>{t('cq_status')}</Text>
                        <Text style={[s.detailValue, { color: statusColor, fontWeight: '700' }]}>
                          {lot.cqlib_status}
                        </Text>
                      </View>
                      {lot.notes && (
                        <View style={s.detailRow}>
                          <Text style={s.detailLabel}>{t('observations')}</Text>
                          <Text style={s.detailValue}>{lot.notes}</Text>
                        </View>
                      )}
